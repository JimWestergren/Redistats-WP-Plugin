...
if( !class_exists( 'Redistats' ) ) 
{
	...
	{ 
		...
		{ 
			...
			add_settings_field(	'redistats_api_key', 'API Key:', array( &$this, 'field_callback' ), 'redistats', 'redistats-section',
				array
				(
					'field' => 'redistats_api_key'
				)
			);
			
			add_settings_field(	'redistats_user_id', 'User ID:', array( &$this, 'field_callback' ), 'redistats', 'redistats-section',
				array
				(
					'field' => 'redistats_user_id'
				)
			);
			
			add_settings_field(	'redistats_email', 'E-Mail:', array( &$this, 'field_callback' ), 'redistats',	'redistats-section',
				array
				(
					'field' => 'redistats_email'
				)
			);
			
			add_settings_field( 'redistats_status', 'Status: ', array( &$this, 'field_callback' ), 'redistats', 'redistats-section',
				array
				(
					'field' => 'redistats_status'
				)
			);
			
			add_settings_field( 'redistats_verification', 'Verification code: ', array( &$this, 'field_callback' ), 'redistats', 'redistats-section',
				array
				(
					'field' => 'redistats_verification'
				)
			);
...
{ 
	...
	if( isset( $redistats) )
	{
		...
		else
		{
			...
			if( get_option( 'redistats_status' ) ) add_action('wp_footer', 'your_redistats');
		}
